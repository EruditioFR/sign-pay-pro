...
export const searchDocuments = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows, error } = await supabase.rpc("search_documents", {
      p_q: data.q?.trim() || undefined,
      p_types: data.types?.length ? data.types : undefined,
      p_statuses: data.statuses?.length ? data.statuses : undefined,
      p_currencies: data.currencies?.length ? data.currencies : undefined,
      p_organization: data.organization_id ?? undefined,
      p_from_date: data.from_date || undefined,
      p_to_date: data.to_date || undefined,
      p_min_amount: data.min_amount ?? undefined,
      p_max_amount: data.max_amount ?? undefined,
