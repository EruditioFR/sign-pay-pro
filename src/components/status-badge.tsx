...
const signerVariant: Record<string, string> = {
  pending: "bg-amber-500/15 text-amber-700 dark:text-amber-300",
  sent: "bg-sky-500/15 text-sky-700 dark:text-sky-300",
  viewed: "bg-violet-500/15 text-violet-700 dark:text-violet-300",
  signed: "bg-indigo-500/15 text-indigo-700 dark:text-indigo-300",
  ...
  cancelled: "bg-zinc-500/20 text-zinc-700 dark:text-zinc-300 line-through",
  expired: "bg-slate-500/15 text-slate-500 dark:text-slate-300",
};

const pdpVariant: Record<string, string> = {
  pending: "bg-amber-500/15 text-amber-700 dark:text-amber-300",
...
const PDP_LABEL: Record<string, string> = {
  pending: "En attente",
  submitted: "Transmise",
...
const SIGNER_LABEL_FALLBACK: Record<string, string> = {
  pending: "En attente",
  sent: "Envoyée",
  viewed: "Consultée",
  signed: "Signée",
...
export function StepStatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant="outline" className={cn("border-0 font-medium", stepVariant[status])}>
      {t(`workflows.step_status.${status}`, { defaultValue: status })}
    </Badge>
  );
}
...
export function SignerStatusBadge({ status }: { status: string }) {
  const { t } = useTranslation();
  return (
    <Badge variant="outline" className={cn("border-0 font-medium", signerVariant[status])}>
      {t(`signatures.status.${status}`, {
        defaultValue: SIGNER_LABEL_FALLBACK[status] ?? status,
      })}
    </Badge>
  );
}

export function PdpStatusBadge({ status }: { status: string }) {
  return (
    <Badge variant="outline" className={cn("border-0 font-medium", pdpVariant[status])}>
      {PDP_LABEL[status] ?? status}
    </Badge>
  );
}
