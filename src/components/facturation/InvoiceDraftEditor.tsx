...
import { ArrowLeft, CheckCircle2, Save, Receipt } from "lucide-react";
import { toast } from "sonner";
import { updateDocument } from "@/lib/documents.functions";
import { saveDocumentLines, getInvoiceFull } from "@/lib/facturation.functions";
...
export function InvoiceDraftEditor({ id }: Props) {
  ...
  const save = useMutation({
    mutationFn: async (issue: boolean) => {
      await persist();
      if (issue) {
        await transitionFn({ data: { documentId: id, to: "issued" } });
      }
    },
    onSuccess: (_d, issue) => {
      toast.success(issue ? "Facture émise." : "Facture enregistrée.");
      ...
      if (issue) {
        // Refresh to show the static post-emission view
        navigate({ to: "/app/facturation/factures/$id", params: { id } });
      }
    },
  ...
  return (
    <div className="space-y-4 max-w-5xl">
      <Button asChild variant="ghost" size="sm">
        <Link to="/app/facturation/factures"><ArrowLeft className="mr-1 h-4 w-4" /> Retour</Link>
      </Button>
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2">
            <Receipt className="h-6 w-6 text-[color:var(--facturation)]" />
            {String(d.document_number ?? "Facture")}
          </h1>
      ...
      <DocumentStepperForm
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            >
              <CheckCircle2 className="mr-1 h-4 w-4" /> Émettre la facture
            </Button>
          </div>
        }
      />
