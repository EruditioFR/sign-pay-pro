  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Kpi icon={FileCheck2} label={t("dashboard.documents_signed")} value={stats?.totals.signed ?? 0} accent="text-emerald-500" />
        <Kpi icon={Euro} label="Montant total" value={fmtAmount(stats?.totals.amountTotal ?? 0)} accent="text-indigo-500" />
      </div>
            ...
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={stats?.last30Days ?? []}>
                  <defs>
                    <linearGradient id="gCreatedA" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="hsl(var(--primary))" stopOpacity={0.4} />
                      <stop offset="100%" stopColor="hsl(var(--primary))" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient id="gSignedA" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="created" name="Créés" stroke="hsl(var(--primary))" fill="url(#gCreatedA)" strokeWidth={2} />
                  <Area type="monotone" dataKey="signed" name="Signés" stroke="#10b981" fill="url(#gSignedA)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <CreditCard className="h-4 w-4 text-indigo-500" /> Paiements
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-4 border-t border-border pt-3">
              ...
              <div className="mt-1 text-xl font-semibold">{fmtAmount(stats?.totals.amountTotal ?? 0)}</div>
            </div>
          </CardContent>
        </Card>
      </div>
...
function Kpi({ icon: Icon, label, value, accent }: { icon: typeof FileText; label: string; value: number | string; accent: string }) {
  return (
    <Card>
